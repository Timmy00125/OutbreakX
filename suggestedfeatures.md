...
## Implementation Status (17 Mar 2026)

- Completed in this pass:
  - Sync markers on load from `/point/all`.
  - Marker management (edit/delete) via map popups.
  - Marker export to GeoJSON and CSV.
  - Search with OpenStreetMap Nominatim.
  - "Find My Location" map centering.
  - Backend point update/delete endpoints (`PUT /point/{id}`, `DELETE /point/{id}`).
- Not yet implemented:
  - Interactive shape drawing (leaflet-draw), proximity buffers, route visualization.
  - Heatmaps, timeline playback, richer server-side filtering.
  - Auth, real-time websocket updates, and backend responsibility consolidation.
